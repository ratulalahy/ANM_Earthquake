...
function initialize() {
  ...
  //heading of page
  document.getElementById('location').innerHTML="Earthquakes Map";
...
function setMarkers(initialize){
  //only do it when map changes location
   google.maps.event.addListenerOnce(map, 'bounds_changed', function() {
      ...
      $.getJSON("http://api.geonames.org/earthquakesJSON?north="+north+"&south="+south+"&east="+east+"&west="+west+"&maxRows=10&username=ratul", function(response){
...
function defineInitialMarkers(response){
  ...
  var EQlist="Top 10 Earthquakes Last 12 Months:<br/>";
